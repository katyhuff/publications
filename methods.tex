...
For reproducibility, all scripts and data used to conduct these simulations are available in \cite{bae_arfc/transition-scenarios:_2017}.
The numerical experiments conducted in this work follow the computational 
workflow in \cref{fig:workflow}.  

\begin{figure}
        \centering
        \scalebox{0.7}{
\begin{tikzpicture}[node distance=1.5cm]
\node (pris) [object] {PRIS data \cite{iaea_nuclear_2017} (\texttt{.csv})};
\node (recipes) [object, left=of pris] {Fuel Recipes (\texttt{.csv})};
\node (simdata) [object, right=of pris] {Simulation Data (\texttt{.py})};
\node (script) [process, below of=pris] {Input Generation Script (\texttt{write\_deployinst\_input.py})};
\node (input) [object, below of=script] {\Cyclus Input File (\texttt{.xml})};
\node (cyclus) [process, below of=input]{\Cyclus};
\node (output) [object, below of=cyclus]{\texttt{Output Database (\texttt{.sqlite})}};
\node (script2) [process, below of=output]{Analysis Script (\texttt{analysis.py})};

\draw [arrow] (recipes) -- (script); 
\draw [arrow] (pris) -- (script); 
\draw [arrow] (simdata) -- (script); 
...
The following sections describe the components of the \cref{fig:workflow} process. 
\Cref{sec:scenario} describes fundamental scenario definition parameters. 
\Cref{sec:eu-deploy} details the simulation input data defining historic and projected nuclear reactor capacities in each \gls{EU} nation from 1970-2050.  
\Cref{sec:reactors} and \cref{sec:materials} describe the reactor and material input data, respectively. 
\Cref{sec:cyclus} describes the \Cyclus simulator and agents used to simulate the fuel cycle transition. 

\subsection{Scenario Specification}\label{sec:scenario}

To calculate the \gls{EU} nuclear material inventory,
we simulated nuclear reactor operating capacity in the \gls{EU}
from 1970 to 2050.
The \gls{EU} \gls{UNF} is stored until France reprocesses it.
The reprocessing and \gls{MOX} fabrication capacity in France prior to 2020 is modeled after the French La Hague and MELOX sites \cite{schneider_spent_2008, hugelmann_melox_1999}.
In 2020, France begins \gls{SFR} fuel production by recycling the stored 
\gls{EU} \gls{UNF}.
In France, existing and planned \glspl{PWR} continue to be fuelled by existing 
\gls{MOX} production until 2040. In 2040, France begins to replace decommissioned reactors 
with \glspl{SFR}, maintaining a 66\gls{GWe} total capacity.
The \glspl{SFR} are modeled after the \gls{ASTRID} breeder reactor \cite{varaine_pre-conceptual_2012}.

Scenario specifications are listed in tables \ref{tab:gen}, \ref{tab:sim_eu}, and \ref{tab:sim_france}.


\begin{table}[h]
    \centering
    \begin{tabularx}{\columnwidth}{bb}
        \hline
        \textbf{Specification} &\textbf{Value} \\
        \hline
        Simulation start & 1970\\ 
        Simulation end & 2160 \\ 
        \gls{ASTRID} fuel production & 2020 \\
        \gls{SFR} available year & 2040 \\
        Storage residence time & $3 [y]$ \\
        \hline
    \end{tabularx}
...
\begin{table}[h]
    ...
    \begin{tabularx}{\columnwidth}{bb}
        ...
        \textbf{Specification} &\textbf{Value} \\
        \hline
        Reprocessing Capacity \cite{schneider_spent_2008} & $91.6 \left[\frac{\mbox{MTHM UNF}}{month}\right]$\\
        Reprocessing Efficiency & 99.8\% \\
        Reprocessing Streams & Pu, U \\
        \gls{MOX} Fabrication  \cite{hugelmann_melox_1999} & $16.25 \left[\frac{\mbox{MTHM}}{month}\right]$\\
        \hline
    \end{tabularx}
...
\section{Reactor Specifications}\label{sec:reactors}
\gls{PWR}, \gls{BWR}, and ASTRID-type \gls{SFR} reactor types are simulated in 
this scenario.
...
\subsection{Material Definitions}\label{sec:materials}
Spent nuclear fuel compositions are recipe-based, such that a fresh 
...
\subsection{\Cyclus}\label{sec:cyclus}
...
\subsection{\gls{EU} Nuclear Deployment}\label{sec:eu-deploy}
...
\cite{hatch_politics_2015}.  The projections extend to 2050 at the latest.  
...
\begin{table}[h]
    ...
    \begin{tabular}{ccccc}
        \hline
            \textbf{Operation} & \textbf{Country} & \textbf{Reactor} & \textbf{Type} & \textbf{Capacity}\\
            \textbf{Start} & & & & [\textbf{\gls{MWe}}]\\
            \textbf{[Year]} & & & &\\
        \hline
...
\subsection{French \gls{SFR} Deployment}\label{sec:fr-deploy}
