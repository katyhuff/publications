...
workflow in \cref{fig:workflow}.  For reproducibility, all scripts and data in that workflow are available in \cite{bae_arfc/transition-scenarios:_2017}.
...
of the process. \Cref{sec:cyclus} describes the \Cyclus simulator and agents used to simulate the fuel cycle transition. \Cref{sec:eu-deploy} 
details the simulation input data defining historic and projected 
nuclear reactor capacities in each \gls{EU} nation from 1970-2050.  
\Cref{sec:reactors} and \cref{sec:materials} describe the reactor and material 
input data, respectively. 
\Cref{sec:scenario} describes additional scenario definition parameters. 
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{bb}
        ...
        \textbf{Specification} &\textbf{Value} \\
        \hline
        Reprocessing Capacity & $91.6 \frac{MTHM UNF}{month}$ \cite{schneider_spent_2008} \\
        Reprocessing Efficiency & 99.8\% \\
        Reprocessing Streams & Pu, U \\
        \gls{MOX} Fabrication  & $16.25 \frac{MTHM}{month}$  \cite{hugelmann_melox_1999} \\
        \hline
    \end{tabularx}
